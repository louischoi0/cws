...
- `POST /milestones/`, `GET /milestones/`, `GET /milestones/{id}/`,
  ...
  was reached.
- `POST /tasks/`, `GET /tasks/` (`?pending=true` and/or `?milestone_id=<id>`
  to filter), `GET /tasks/{id}/` — the task queue (see below). A task may
...
and reported on across Claude Code sessions. A `task` (`version`, `title`,
`content` in markdown, an open-ended `type`, and KST timestamps for when
it was raised and last shipped) describes one unit of work; a
`result` (`status`, `content` in markdown, `completed_at`) records what
one attempt at it produced — a task can accumulate many results over its
life, not just one.

The **intermediary agent** (`.claude/agents/intermediary-agent.md`) is the
worker on the other end: it polls `GET /tasks/?pending=true`, picks one
up, and does the actual work *inside the target project*, following
*that project's own* `CLAUDE.md` and subagents rather than any process of
its own — this repo only owns the reporting contract, never how the work
itself gets done. When it's done (or gives up), it reports back via
`POST /tasks/{id}/results/`, which also marks the task as shipped. It then
